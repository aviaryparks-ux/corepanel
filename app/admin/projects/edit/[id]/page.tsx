...
export default function EditProjectPage() {
  ...
  const fetchProject = async () => {
    try {
      ...
      const project = data.find((p: Project) => p.id === parseInt(id as string));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(errorData.error || "Gagal mengupdate project");
      }
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        ...
        <Link href="/admin/projects" className="inline-block mt-4 bg-blue-600 text-white px-4 py-2 rounded">
